...
func (s *Service) saveWithType(ctx context.Context, title, content, source, sourceRef string, tags []string, knowledgeType string) (*SaveResult, error) {
	...
	if result.Saved && knowledgeType != "" && knowledgeType != domain.TypeGeneral {
		if _, err := s.UpdateKnowledge(ctx, result.ID, "", "", nil, knowledgeType); err != nil {
			slog.Warn("failed to set knowledge_type", "id", result.ID, "type", knowledgeType, "error", err)
		}
	}
	return result, nil
}
