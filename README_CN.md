...
知识库采用 **类 Git 分支模型** —— 新知识进入 `pending`（暂存区），经人工审核后变为 `approved`（主分支），只有通过审核的知识才会被搜索召回。

- **人工审核** —— 每条知识必须经过人工确认才能进入可搜索状态，杜绝错误知识污染全员
- **LLM 辅助建议** —— LLM 提供质量评估建议，但最终决定权在人
- **置信度评分** —— 审核通过的知识带 0-100 置信度
- **Web UI 审核面板** —— 表格视图 + 状态 Tab（全部/待审核/已通过/已拒绝），支持内联编辑、一键通过/拒绝

### Claude Code Skill 体系

4 个内置 Skill，让 AI 智能体高效使用知识库：

| Skill | 触发方式 | 作用 |
|-------|---------|------|
| `/know-save` | "记住这个"、"保存知识" | 从对话中提取知识并保存 |
| `/know-search` | **自动触发**：决策/规则/架构相关问题 | 回答前先搜知识库 |
| `/know-capture` | "提取这段对话的知识" | 批量提取对话中的多条知识 |
| `/know-review` | "审核知识" | 交互式审核待审队列 |

**`/know-search` 支持主动触发** —— 当用户询问历史决策、业务规则、架构选型时，AI 自动搜索知识库再回答，无需用户手动要求。

### MCP 可观测性

- **MCP 调用日志** —— 追踪每次 `note_search` 调用，区分 MCP / Web 来源
- **知识调用排行** —— 哪些知识被召回最多
- **不准确标记** —— 标记错误的搜索结果，持续改进知识质量
- **准确率指标** —— 基于反馈计算每条知识的准确率
...
- **MCP 原生集成** —— AI 智能体通过 `note_save` 和 `note_auto_capture` 直接保存学到的知识
...
- **8 个 MCP 工具** —— 完整对接 Claude Code、Cursor 及所有 MCP 兼容客户端
- **4 个 Claude Code Skill** —— 开箱即用的技能，提升 MCP 使用率
- **REST API** —— 完整的 CRUD + 搜索 + 审核 + 监控接口
- **内置 Web UI** —— 知识审核、调用监控、标签云、维护面板
...
| 工具 | 说明 |
|------|------|
| `note_search` | 语义搜索知识库（仅返回已审核通过的知识） |
| `note_save` | 保存知识，自动去重和关联（新知识进入待审核） |
| `note_auto_capture` | 通过 LLM 从对话会话中提取知识 |
| `note_import` | 导入文档，按 Markdown 标题自动分块 |
| `note_update` | 更新已有知识条目 |
| `note_review` | 审核待审知识：列表 / 通过 / 拒绝 / 打回 |
...
```
 ...
 │          │    │          │    │          │    │ (暂存区)  │    │ (主分支)  │
 │ • 对话   │    │  OpenAI  │    │ ≥0.95 ↻  │    │          │    │          │
 │ • 文档   │    │ embedding│    │ 0.75 🔗  │    │  人工审核  │    │ MCP 可召回│
 │ • 手动   │    │          │    │ <0.75 ✚  │    │          │    │          │
 └──────────┘    └──────────┘    └──────────┘    └──────────┘    └──────────┘
                                                                       │
                        ┌──────────────────────────────────────────────┘
...
| POST | `/api/knowledge` | 创建知识条目（进入待审核） |
| GET | `/api/knowledge/:id` | 获取单个条目 |
| PUT | `/api/knowledge/:id` | 更新条目 |
| DELETE | `/api/knowledge/:id` | 删除条目 |
| GET/POST | `/api/search` | 搜索知识库（仅返回已审核通过的知识） |
| GET/POST | `/api/review` | 待审列表 / 通过 / 拒绝 / 打回 |
| POST | `/api/import` | 导入文档 |
| POST | `/api/capture` | 捕获会话知识 |
| POST | `/api/feedback` | 记录有用反馈 |
| GET/POST | `/api/maintain` | 查看或执行维护任务 |
| GET | `/api/stats` | 知识库统计（含审核计数） |
| GET | `/api/search_log` | 搜索日志分析 |
| GET | `/api/monitor/ranking` | 知识调用排行（含准确率） |
| GET | `/api/monitor/logs` | MCP 调用日志（可按来源过滤） |
| POST | `/api/monitor/bad_recall` | 标记搜索结果不准确 |
...
├── cmd/server/            # 入口 & 依赖组装
├── internal/
│   ├── domain/            # 领域模型（Knowledge、SearchHit、ReviewResult 等）
│   ├── port/              # 接口定义（Store、Retriever、Embedder 等）
│   ├── service/           # 业务逻辑编排
│   └── adapter/
│       ├── api/           # REST API 路由（含审核 & 监控）
...
│   ├── know-save/         # 保存知识
│   ├── know-search/       # 搜索知识（支持自动触发）
│   ├── know-capture/      # 批量提取对话知识
│   └── know-review/       # 交互式审核队列
├── web/                   # 嵌入式静态 Web UI
├── config.json.example
├── docker-compose.yml
...
- [x] 内置 Web UI
- [x] **知识质量体系** —— 类 Git 审核流（pending → approved → 可搜索）
- [x] **Claude Code Skill 体系** —— 4 个技能提升 MCP 使用率（`/know-save`、`/know-search`、`/know-capture`、`/know-review`）
- [x] **MCP 可观测性** —— 调用日志、命中排行、不准确标记、准确率指标
- [ ] 多用户支持 + 身份认证
- [ ] 浏览器插件：一键捕获网页知识
- [ ] 移动端 App（iOS / Android）
